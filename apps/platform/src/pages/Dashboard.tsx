...
import { websiteService, snapshotService, accessibilityService, memberService } from "../services/api";
...
export default function Dashboard() {
  ...
  const {
    ...
    queryFn: async (): Promise<Website[]> => {
      ...
      return Array.isArray(data) ? data as Website[] : [];
    },
    enabled: !!token,
  });
  ...
  const { data: snapshots = [], isLoading: isLoadingSnapshots } = useQuery({
    ...
    queryFn: async (): Promise<Snapshot[]> => {
      ...
      const snapshotPromises = websites
        ...
        .map(async (website) => {
          try {
            const data = await snapshotService.getSnapshots(website.id as string);
  ...
  const { data: accessibilityIssues = [], isLoading: isLoadingAccessibility } = useQuery({
    ...
    queryFn: async (): Promise<AccessibilityIssue[]> => {
      ...
      const issuePromises = websites
        ...
        .map(async (website) => {
          try {
            const data = await accessibilityService.getAccessibilityResults(website.id as string);
            if (Array.isArray(data)) return data as AccessibilityIssue[];
            return data?.issues || [];
          } catch (error) {
            console.warn(`Failed to fetch accessibility issues for website ${website.id}`);
            return [];
          }
        });
  ...
  const isLoading = isLoadingWebsites || isLoadingSnapshots || isLoadingAccessibility || isLoadingMembers;
        ...
        <div className="relative z-10 flex items-center justify-center min-h-screen p-4">
          <div className="max-w-md w-full">
            <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-green-100/50 p-8 text-center">
              ...
              <p className="text-slate-600 mb-8">Please sign in to view your monitoring dashboard.</p>
              <Link to="/login" className="inline-flex items-center gap-3 px-8 py-4 rounded-xl bg-gradient-to-r from-green-600 to-emerald-600 text-white font-semibold shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-200">
                Sign In to Continue
              </Link>
            </div>
          </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-green-50/30 to-emerald-50/50 relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-30">
        <div style={{ backgroundImage: "linear-gradient(rgba(34, 197, 94, 0.1) 1px, transparent 1px), linear-gradient(90deg, rgba(34, 197, 94, 0.1) 1px, transparent 1px)", backgroundSize: "40px 40px" }} />
      </div>
        ...
        {isLoading ? (
          <div className="flex items-center justify-center py-16">
            <div className="text-center">
              ...
              <h2 className="text-2xl font-bold font-heading text-slate-800 mb-2">Loading Dashboard</h2>
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              ...
              <div className="group relative overflow-hidden bg-gradient-to-br from-white/90 to-white/60 backdrop-blur-2xl rounded-3xl border border-white/60 shadow-2xl hover:shadow-3xl hover:-translate-y-2 transition-all duration-500 p-6">
                ...
                <div className="relative z-10">
                  ...
                  <h3 className="text-sm font-semibold text-slate-700 mb-1 tracking-wide">WEBSITES</h3>
                  <p className="text-4xl font-bold bg-gradient-to-r from-blue-600 to-blue-700 bg-clip-text text-transparent mb-3">{websites.length}</p>

                  <div className="flex items-center justify-between mb-4 pb-4 border-b border-slate-200/40">
                    <div className="space-y-1">
                      <p className="text-xs text-slate-600"><span className="font-bold text-emerald-600">{activeWebsites}</span> active</p>
                      <p className="text-xs text-slate-600"><span className="font-bold text-amber-600">{inactiveWebsites}</span> inactive</p>
                    </div>
                  </div>

                  {snapshotsOverTime.length > 0 && (
                    <div className="h-12">
                      <p className="text-xs text-slate-500 mb-2 font-medium">7-day trend</p>
              ...
              <div className="group relative overflow-hidden bg-gradient-to-br from-white/90 to-white/60 backdrop-blur-2xl rounded-3xl border border-white/60 shadow-2xl hover:shadow-3xl hover:-translate-y-2 transition-all duration-500 p-6">
                ...
                <div className="relative z-10">
                  ...
                  <h3 className="text-sm font-semibold text-slate-700 mb-1 tracking-wide">SNAPSHOTS</h3>
                  <p className="text-4xl font-bold bg-gradient-to-r from-purple-600 to-purple-700 bg-clip-text text-transparent mb-3">{snapshots.length}</p>

                  <div className="flex items-center justify-between mb-4 pb-4 border-b border-slate-200/40">
                    <div className="space-y-1">
                      <p className="text-xs text-slate-600">Last snapshot: <span className="font-bold text-slate-800">{snapshots.length > 0 ? new Date(snapshots[0]?.capturedAt).toLocaleDateString() : "N/A"}</span></p>
                    </div>
                  </div>
              ...
              <div className="group relative overflow-hidden bg-gradient-to-br from-white/90 to-white/60 backdrop-blur-2xl rounded-3xl border border-white/60 shadow-2xl hover:shadow-3xl hover:-translate-y-2 transition-all duration-500 p-6">
                ...
                <div className="relative z-10">
                  ...
                  <h3 className="text-sm font-semibold text-slate-700 mb-1 tracking-wide">ISSUES</h3>
                  <p className="text-4xl font-bold bg-gradient-to-r from-red-600 to-red-700 bg-clip-text text-transparent mb-3">{criticalAccessibilityIssues}</p>

                  <div className="flex items-center justify-between mb-4 pb-4 border-b border-slate-200/40">
                    <div className="space-y-1">
                      <p className="text-xs text-slate-600"><span className="font-bold text-red-600">{criticalAccessibilityIssues}</span> critical/high</p>
                      <p className="text-xs text-slate-600"><span className="font-bold text-slate-800">{totalIssues}</span> total</p>
                    </div>
                  </div>
              ...
              <div className="group relative overflow-hidden bg-gradient-to-br from-white/90 to-white/60 backdrop-blur-2xl rounded-3xl border border-white/60 shadow-2xl hover:shadow-3xl hover:-translate-y-2 transition-all duration-500 p-6">
                ...
                <div className="relative z-10">
                  ...
                  <h3 className="text-sm font-semibold text-slate-700 mb-1 tracking-wide">TEAM MEMBERS</h3>
                  <p className="text-4xl font-bold bg-gradient-to-r from-emerald-600 to-emerald-700 bg-clip-text text-transparent mb-3">{members.length}</p>

                  <div className="flex items-center justify-between mb-4 pb-4 border-b border-slate-200/40">
                    <div className="space-y-1">
                      <p className="text-xs text-slate-600"><span className="font-bold text-emerald-600">{members.filter(m => m.type === "Admin").length}</span> admin</p>
                      <p className="text-xs text-slate-600"><span className="font-bold text-slate-800">{members.filter(m => m.type !== "Admin").length}</span> members</p>
                    </div>
                  </div>

                  {membersOverTime.length > 0 && (
                    <div className="h-12">
                      <p className="text-xs text-slate-500 mb-2 font-medium">Growth</p>
                      <ResponsiveContainer width="100%" height={32}>
            ...
            {websites.length > 0 ? (
              <div className="mb-8">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <h2 className="text-2xl font-bold font-heading text-slate-900">Your Websites</h2>
                    <p className="text-slate-600 mt-1">{websites.length} tracked properties</p>
                  </div>
                </div>
                  ...
                  {websites.map((website) => (
                    ...
                    >
                      ...
                      <div className="relative z-10">
                        ...
                        <p className="text-sm text-slate-500 truncate mb-4">{website.url}</p>

                        <div className="space-y-2 pt-4 border-t border-slate-200/40">
                          <div className="flex items-center gap-2 text-sm">
                            <Calendar className="w-4 h-4 text-slate-400" />
                            <span className="text-slate-600">Created: {new Date(website.createdAt).toLocaleDateString()}</span>
                          </div>
                          {website.latestSnapshot && (
                            <div className="flex items-center gap-2 text-sm">
                              <Camera className="w-4 h-4 text-emerald-500" />
                              <span className="text-emerald-700 font-medium">Last: {new Date(website.latestSnapshot).toLocaleDateString()}</span>
                            </div>
                          )}
                        </div>
                      </div>
              ...
              <div className="text-center py-16 mb-8">
                <div className="bg-gradient-to-br from-slate-100/60 via-white/40 to-slate-100/60 backdrop-blur-xl rounded-3xl p-12 inline-block border border-slate-200/50">
                  ...
                  <h2 className="text-2xl font-bold font-heading text-slate-800 mb-2">No websites tracked yet</h2>
                  <p className="text-slate-600">Start monitoring by capturing a snapshot using the browser extension.</p>
                </div>
              </div>
            )}
            ...
            {members.length > 0 && (
              <div className="mb-12">
                <h2 className="text-2xl font-bold font-heading text-slate-900 mb-6">Team Members</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {members.map((member) => (
                    <Link
                      key={member.memberId}
                      ...
                      <div className="relative z-10">
                        <div className="flex items-start justify-between mb-4">
                          <div>
                            ...
                            <p className="text-sm text-slate-600 capitalize mt-1">{member.role}</p>
                          </div>
                          <div className={`px-3 py-1 rounded-full text-xs font-semibold ${member.type === "Admin"
                        ...
                        <div className="pt-4 border-t border-slate-200/40">
                          <div className="flex items-center gap-2 text-sm">
                            ...
                            <span className="text-slate-600">{member.type === "Admin" ? "Administrator" : "Team Member"}</span>
                          </div>
                        </div>
