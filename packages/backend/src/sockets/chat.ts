  ...
  io.on('connection', async (socket: Socket) => {
    ...
    socket.on('typing:start', async (payload: { room: 'group' | 'dm'; peerMemberId?: string }) => {
      try {
        if (payload.room === 'group') {
          ...
          const recipient = await Member.findOne({ memberId: toMemberId, userId }).lean();
          if (!recipient) return;
          // notify peer that memberId is typing in DM
    ...
    socket.on('dm:read', async (payload: { peerMemberId: string; lastCreatedAt: string }) => {
      try {
        ...
        const recipient = await Member.findOne({ memberId: peerMemberId, userId }).lean();
        if (!recipient) return;
        ...
        await ChatMessage.updateMany(
          {
            ...
            type: 'dm',
            conversationId,
            fromMemberId: peerMemberId, // messages from peer to me
            createdAt: { $lte: lastCreatedAt },
          },
          { $addToSet: { readBy: memberId } }
        );
        // Notify sender (peer) so they can update ticks
    ...
    socket.on('dm:send', async (payload: { toMemberId: string; content: string }) => {
      try {
        ...
        // Validate recipient belongs to same organization
        const recipient = await Member.findOne({ memberId: toMemberId, userId }).lean();
        if (!recipient) return; // silently ignore invalid target
        ...
        // Emit to both sender and recipient personal rooms
    ...
    socket.on('disconnect', async () => {
      try {
        ...
        const connKey = `conn:${memberId}`;
        const nowIso = new Date().toISOString();
        const remaining = await redis.decr(connKey);
        await redis.set(`last_seen:${memberId}`, nowIso);
        if (remaining <= 0) {
        ...
        console.error('presence teardown error', e);
      }
    });
  });
};
