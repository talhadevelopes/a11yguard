...
export function CodeComparison({
  ...
  // Map common theme names to imported theme objects to ensure styles are available in-browser
  const selectedThemeObject = useMemo(() => {
    // Temporarily force a single theme to avoid mismatch issues during testing
    return githubLight as any;
  }, [selectedTheme]);
    ...
    async function highlightCode() {
      try {
        ...
        const before = await codeToHtml(beforeCode, {
          lang: language,
          theme: selectedThemeObject as any,
          transformers: [
    ...
    if (highlighted) {
      return (
        <div
          ...
          className={cn(
            "h-full w-full overflow-auto bg-background font-mono text-xs overflow-x-auto",
            "[&>pre]:h-full [&>pre]:!w-screen [&>pre]:py-2",
            "[&>pre>code]:!inline-block [&>pre>code]:!w-full [&>pre>code]:whitespace-pre [&>pre>code]:break-normal",
            ...
            // Ensure diff lines are visibly colored regardless of theme (Shiki uses .add/.remove on diff lines)
            "[&>pre>code>.add]:bg-[rgba(16,185,129,.30)] [&>pre>code>.add]:text-inherit [&>pre>code>.add]:border-l-2 [&>pre>code>.add]:border-emerald-500",
            "[&>pre>code>.remove]:bg-[rgba(244,63,94,.30)] [&>pre>code>.remove]:text-inherit [&>pre>code>.remove]:border-l-2 [&>pre>code>.remove]:border-rose-500",
      ...
      return (
        <pre className="h-full overflow-auto break-all bg-background p-4 font-mono text-xs">
          {code}
        </pre>
      );
    }
